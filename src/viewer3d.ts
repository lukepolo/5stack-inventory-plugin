...
import { API_ORIGIN, getAssetOrigin } from "./api";
...
export const modelUrlFor = (model: string) =>
  `${getAssetOrigin()}/models/${model.split("/").map(encodeURIComponent).join("/")}.glb`;
...
async function loadGltf(model: string) {
  ...
  if (model.includes("/")) loader.setResourcePath(`${getAssetOrigin()}/models/`);
